...
const navGroups: NavGroup[] = [
  ...
  },
  {
    ...
    icon: "pen-tool",
    items: [
      { name: "Texnik chizmalar", href: "/technicaldrawings", icon: "blueprint", permission: ["TechnicalDrawings.View", "TechnicalDrawings.ViewAll"] },
    ],
  },
  {
    label: "Omborxona",
...
function NavIcon({ type, size = 16, strokeWidth = 2, color }: { type: string; size?: number; strokeWidth?: number; color?: string }) {
  ...
  if (type === "blueprint")    return <svg className={cls} width={size} height={size} fill="none" {...s} strokeWidth={strokeWidth} viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="8" x2="21" y2="8"/><line x1="8" y1="3" x2="8" y2="21"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="18" x2="13" y2="18"/><polygon points="13,10 17,10 17,12 13,12" strokeWidth="1.5"/><circle cx="18" cy="18" r="2" strokeWidth="1.5"/></svg>;
  if (type === "pen-tool")     return <svg className={cls} width={size} height={size} fill="none" {...s} strokeWidth={strokeWidth} viewBox="0 0 24 24"><path d="M12 19l7-7 3 3-7 7-3-3z"/><path d="M18 13l-1.5-7.5L2 2l3.5 14.5L13 18l5-5z"/><path d="M2 2l7.586 7.586"/><circle cx="11" cy="11" r="2"/></svg>;
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const visibleNavGroups = directorMode
    ? [{ label: "Asosiy", icon: "layout", items: [{ name: "Dashboard", href: "/", icon: "grid" }] }]
    : navGroups.map((group) => ({
        ...group,
        items: group.items
          .filter((item) => {
  ...
  useEffect(() => {
    ...
    // Direktor faqat dashboard'ga kirishi mumkin
    if (directorMode && !isLoginPage && pathname !== "/") {
      router.replace("/");
    }
  }, [hasHydrated, isLoginPage, accessToken, router, directorMode, pathname]);
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          <div style={{
            ...
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
              ...
              {/* Notification bell */}
              {!directorMode && <div style={{ position: "relative" }}>
                <button
                  className={darkMode ? "theme-btn-sun" : "theme-btn-moon"}
